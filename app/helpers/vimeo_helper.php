...
function getVimeo($id, $quality = "sd") {
    // get page with a player
    $queryResult = httpQuery('http://player.vimeo.com/video/' . $id);
	...
	if($quality != "sd"  && $quality != "hd" && $quality != "mobile")
	{
		$quality = "sd";
	}

    $url = 'http://player.vimeo.com/play_redirect?clip_id=' . $id . '&sig=' . $signature . '&time=' . $timestamp . '&quality='.$quality;//y=sd  y=hd y=mobile
