...
class Opciones_model extends CI_model {
	...
	function add($data){
        $this->db->insert('seg_opciones', $data);
	...
	function edit($data,$fieldID,$ID){
        ...
        $this->db->update('seg_opciones', $data);
	...
	function get_opciones_combo($fields,$where='')
	{
		$this->db->select($fields);
		$this->db->from('seg_opciones');
		if($where)
	...
	function count(){
		return $this->db->count_all('seg_opciones');
	}
}