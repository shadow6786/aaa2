<div class="section_header">
    <h3>Features</h3>
</div> 
<div class="row feature">
    <div class="col-sm-6">
        <img src="<?php echo cdn_base("img/showcase1.png"); ?>" class="img-responsive" />
    </div>
    <div class="col-sm-6 info">
        <h3>
        <img src="<?php echo cdn_base("img/features-ico1.png"); ?>" />
            Beautiful on all devices
        </h3>
        <p>
            There are many variations of passages of Lorem Ipsum available, but the randomised words which don't look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in the middle of text.
        </p>
    </div>
</div>
<div class="row feature">
    <div class="col-sm-6 pic-right">
        <img src="<?php echo cdn_base("img/showcase2.png"); ?>" class="pull-right img-responsive" />
    </div>
    <div class="col-sm-6 info info-left">
        <h3>
            <img src="<?php echo cdn_base("img/features-ico2.png"); ?>" />
            Blog page included
        </h3>
        <p>
            There are many variations of passages of Lorem Ipsum available, but the randomised words which don't look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in the middle of text.
        </p>
    </div>                
</div>
<div class="row feature">
    <div class="col-sm-6">
        <img src="<?php echo cdn_base("img/showcase3.png"); ?>" class="img-responsive" />
    </div>
    <div class="col-sm-6 info">
        <h3>
            <img src="<?php echo cdn_base("img/features-ico3.png"); ?>" />
            Simple and clean coming soon page
        </h3>
        <p>
            There are many variations of passages of Lorem Ipsum available, but the randomised words which don't look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in the middle of text.
        </p>
    </div>
</div>